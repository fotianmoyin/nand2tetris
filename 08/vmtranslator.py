...
class Vmtranslator:
    ...
    def translation(self, args1: str):
        ...
        for vm_file in vm_files:
            ...
            if not success:
                logging.error(f"编译 {vm_file} 失败")
                break
            logging.info(f"编译 {vm_file} 完成")
        self.code_writer.close()
        if success:
            print("编译完成")
            logging.info(f"编译 {asm_name}.asm 成功")
        pass
    ...
    def parse_vm_file(self, vm_folder: str, vm_file: str) -> bool:
        ...
        编译vm文件
        参数：
        vm_folder(str)：vm文件所在目录
        vm_file(str)：vm文件名
        返回值：
        bool：编译成功，返回true；否则，返回false
        """
        logging.info(f"编译 {vm_file} 开始")
        ...
        try:
            ...
            match command_type:
                ...
                case CommandTypes.C_CALL:
                    function_name = vm_parser.arg1()
                    num_args = int(vm_parser.arg2())
                    self.code_writer.write_call(function_name, num_args)
